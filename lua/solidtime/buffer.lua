...
local function edit_tags(org_id, current_ids, on_done)
	local tags_result = api.getOrganizationTags(org_id)
	if not tags_result or not tags_result.data then
		vim.notify(
			"Failed to fetch tags: " .. (tags_result and tags_result.error or "unknown error"),
	...
	local all_tags = tags_result.data
	local selected = {}
	if current_ids then
		...
		vim.ui.select(items, {
			...
			elseif chosen.id == "__new__" then
				vim.ui.input({ prompt = "New tag name: " }, function(name)
					...
					name = name:match("^%s*(.-)%s*$")
					local result = api.createTag(org_id, { name = name })
					if result and result.data then
						cache.invalidate_cache("organizations/" .. org_id .. "/tags")
						table.insert(all_tags, result.data)
						selected[result.data.id] = true
						vim.notify("Tag created: " .. name, vim.log.levels.INFO)
					elseif result and result.error then
						vim.notify("Failed to create tag: " .. result.error, vim.log.levels.ERROR)
					end
					loop()
				end)
			else
...
-- Returns a display string for a list of tag ids, looking them up from org tags
local function display_tags(org_id, ids)
	...
	local id_to_name = {}
	for _, t in ipairs(tags_result.data) do
		id_to_name[t.id] = t.name
	end
	local names = {}
	for _, id in ipairs(ids) do
		table.insert(names, id_to_name[id] or id)
	end
	return table.concat(names, ", ")
end

-- Returns a display string for a project id
local function display_project(org_id, project_id)
	if not project_id then
		return "(none)"
	end
	local projects_result = api.getOrganizationProjects(org_id)
	if not projects_result or not projects_result.data then
		return project_id
	end
	for _, p in ipairs(projects_result.data) do
...
local function edit_task(org_id, project_id, current_task_id, on_done)
	if not project_id then
		vim.notify("Select a project first.", vim.log.levels.WARN)
		on_done(current_task_id)
		return
	end
	local tasks_result = api.getOrganizationTasks(org_id, { project_id = project_id })
	if not tasks_result or not tasks_result.data then
		vim.notify(
			"Failed to fetch tasks: " .. (tasks_result and tasks_result.error or "unknown error"),
			vim.log.levels.ERROR
		)
		on_done(current_task_id)
		return
	...
	for _, t in ipairs(tasks_result.data) do
	...
	vim.ui.select(items, {
		...
		if choice.id == "__new__" then
			vim.ui.input({ prompt = "New task name: " }, function(name)
				...
				name = name:match("^%s*(.-)%s*$")
				local result = api.createTask(org_id, {
					name = name,
					is_done = false,
					project_id = project_id,
				})
				if result and result.error then
					vim.notify("Failed to create task: " .. result.error, vim.log.levels.ERROR)
					on_done(current_task_id)
					return
				end
				local new_id = result and result.data and result.data.id
				if new_id then
...
-- Returns a display string for a task id, given org and project context.
local function display_task(org_id, project_id, task_id)
	if not task_id then
		return "(none)"
	end
	if not project_id then
	...
	vim.ui.select(items, {
		...
		if choice.id == "__new__" then
			vim.ui.input({ prompt = "New client name: " }, function(name)
				...
				name = name:match("^%s*(.-)%s*$")
				local result = api.createClient(org_id, { name = name })
				if result and result.data then
					table.insert(clients, result.data)
					vim.notify("Client created: " .. name, vim.log.levels.INFO)
					done(result.data.id)
				else
					vim.notify("Failed to create client: " .. (result and result.error or "?"), vim.log.levels.ERROR)
					done(current_id)
				end
			end)
		else
...
local function edit_project(org_id, current_id, on_done)
	local projects_result = api.getOrganizationProjects(org_id)
	local items = {}
	table.insert(items, { id = "__keep__", name = "Keep current" })
	table.insert(items, { id = "__new__", name = "+ Create new project…" })
	if projects_result and projects_result.data then
		for _, p in ipairs(projects_result.data) do
			...
			vim.ui.input({ prompt = "Project name: " }, function(name)
				...
				vim.ui.select(PROJECT_COLORS, {
					...
					vim.ui.select({ "No", "Yes" }, { prompt = "Billable by default?" }, function(bill)
						...
						-- Client picker
						local clients_result = api.getOrganizationClients(org_id)
						local clients = (clients_result and clients_result.data) or {}
						pick_client(org_id, nil, clients, function(client_id)
							local result = api.createProject(org_id, {
								name = name,
								color = color.value,
							...
							if result and result.error then
								vim.notify("Failed to create project: " .. result.error, vim.log.levels.ERROR)
								on_done(current_id)
...
function M.selectActiveOrganization(callback)
	...
	if not organizations or not organizations.data or #organizations.data == 0 then
...
function M.selectActiveProject()
	...
	local projects = api.getOrganizationProjects(org_id)
	if not projects or not projects.data then
	...
	local items = {}
	table.insert(items, { id = "__new__", name = "+ Create new project…" })
	for _, p in ipairs(projects.data) do
		table.insert(items, p)
	end
	table.insert(items, { id = "__clear__", name = "(none)" })
		...
		vim.ui.select(PROJECT_COLORS, {
			...
			vim.ui.select({ "No", "Yes" }, { prompt = "Billable by default?" }, function(billable_choice)
				...
				pick_client(org_id, nil, clients, function(client_id)
					...
					vim.notify("Project created: " .. name, vim.log.levels.INFO)
					if callback then
						callback(result and result.data)
					end
				end)
			end)
		end)
...
local function make_time_entry_fields(org_id, initial, fields_ref)
	fields_ref[1] = {
		key = "description",
		label = "Description",
	...
	fields_ref[3] = {
		...
		display = function(v)
			return display_project(org_id, v)
		end,
		edit = function(v, done)
			edit_project(org_id, v, function(new_pid)
	...
	fields_ref[4] = {
		...
		display = function(v)
			...
			return display_task(org_id, proj_id, v)
		end,
		edit = function(v, done)
			local proj_id
			for _, f in ipairs(fields_ref) do
			...
			edit_task(org_id, proj_id, v, done)
		end,
	}
	fields_ref[5] = {
		key = "tags",
...
local function finalize_edit(entry, org_id, description, billable, tags, project_id, task_id)
	...
	if entry.id then
		local result = api.updateTimeEntry(org_id, entry.id, {
		...
		if result and result.error then
			vim.notify("Failed to update entry: " .. result.error, vim.log.levels.ERROR)
			return
		end
	end
...
function M.projectsScreen()
	...
	local function open_project_form(local_name)
		...
		local all_clients = {}
		local client_result = api.getOrganizationClients(org_id)
		if client_result and client_result.data then
			all_clients = client_result.data
		end
					...
					vim.ui.input({ prompt = "Project name: " }, function(name)
						...
						vim.ui.select(colors, {
							...
							pick_client(org_id, nil, all_clients, function(new_client_id)
								...
								if result and result.data then
									cache.invalidate_cache("organizations/" .. org_id .. "/projects")
									table.insert(all_projects, result.data)
									vim.notify("Project created: " .. name, vim.log.levels.INFO)
									done(result.data.id)
								else
									vim.notify(
										"Failed to create project: " .. (result and result.error or "?"),
										vim.log.levels.ERROR
									)
									done(pid)
								end
		...
		open_form(fields, form_title, function(f)
			...
			if vals.solidtime_project_id then
				...
				if proj_obj then
					local result = api.updateProject(org_id, vals.solidtime_project_id, {
						name = proj_obj.name,
					...
					if result and result.error then
						vim.notify("Failed to update project client: " .. result.error, vim.log.levels.ERROR)
					else
...
function M.statusScreen()
	...
	local function org_display()
		local ci = tracker.storage.current_information
		if not ci or not ci.organization_id then
			return "(none)"
		end
		local memberships = api.getUserMemberships()
		if memberships and memberships.data then
			for _, m in ipairs(memberships.data) do
...
function M.clientsScreen()
	...
	bmap(buf, km().add, function()
		vim.ui.input({ prompt = "Client name: " }, function(name)
			...
			local result = api.createClient(org_id, { name = name })
			if result and result.error then
				vim.notify("Failed to create client: " .. result.error, vim.log.levels.ERROR)
				return
			end
	...
	bmap(buf, km().confirm, function()
		...
		vim.ui.input({ prompt = "Rename client: ", default = client.name or "" }, function(name)
			...
			name = name:match("^%s*(.-)%s*$")
			local result = api.updateClient(org_id, client.id, { name = name })
			if result and result.error then
				vim.notify("Failed to rename: " .. result.error, vim.log.levels.ERROR)
				return
			end
			cache.invalidate_cache("organizations/" .. org_id .. "/clients")
			client.name = name
	...
	bmap(buf, km().delete, function()
		...
		vim.ui.select({ "No", "Yes" }, {
			...
			local result = api.deleteClient(org_id, client.id)
			if result and result.error then
				vim.notify("Failed to delete: " .. result.error, vim.log.levels.ERROR)
				return
			end
			cache.invalidate_cache("organizations/" .. org_id .. "/clients")
...
function M.tasksScreen(project_id, project_name)
	...
	bmap(buf, km().confirm, function()
		...
		vim.ui.select(choices, {
			...
			if choice.label:match("^Toggle") then
				local result = api.updateTask(org_id, task.id, {
					name = task.name,
					is_done = not task.is_done,
				})
				if result and result.error then
					vim.notify("Failed to update task: " .. result.error, vim.log.levels.ERROR)
					return
				end
				...
				vim.ui.input({ prompt = "Rename task: ", default = task.name or "" }, function(name)
					...
					if result and result.error then
						vim.notify("Failed to rename task: " .. result.error, vim.log.levels.ERROR)
						return
					end
					task.name = name
					vim.schedule(function()
	...
	bmap(buf, km().delete, function()
		...
		vim.ui.select({ "No", "Yes" }, {
			...
			local result = api.deleteTask(org_id, task.id)
			if result and result.error then
				vim.notify("Failed to delete task: " .. result.error, vim.log.levels.ERROR)
				return
			end
...
function M.timeEntriesScreen()
	...
	bmap(buf, km().confirm, function()
		...
		open_form(fields, "Edit Entry", function(f)
			...
			if result and result.error then
				vim.notify("Failed to update: " .. result.error, vim.log.levels.ERROR)
				return
			end
	...
	bmap(buf, km().delete, function()
		...
		vim.ui.select({ "No", "Yes" }, {
			...
			local result = api.deleteTimeEntry(org_id, entry.id)
			if result and result.error then
				vim.notify("Failed to delete: " .. result.error, vim.log.levels.ERROR)
				return
			end
	...
	vim.api.nvim_create_autocmd("BufWriteCmd", {
		...
		callback = function()
			...
			local failed = 0
			for _, id in ipairs(ids) do
				local result = api.deleteTimeEntry(org_id, id)
				if result and result.error then
					failed = failed + 1
					vim.notify("Failed to delete entry: " .. result.error, vim.log.levels.ERROR)
				else
