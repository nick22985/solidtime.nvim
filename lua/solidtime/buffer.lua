...
--- Register a plugin tab.  Called by the plugin system during setup.
---@param tab_def { id: string, label: string, render: function }
function M.register_tab(tab_def)
	for _, t in ipairs(TABS) do
		if t.id == tab_def.id then
			return -- already registered
		end
	end
	table.insert(TABS, tab_def)
end
...
---@param tab string|integer  tab id (e.g. "timer","status","projects","clients","tasks","entries") or 1-based index
...
-- ---------------------------------------------------------------------------
-- Exposed internals for plugin UI modules.
-- Plugins use these to render content inside the shared floating window.
-- ---------------------------------------------------------------------------
M._shell = shell
M._shell_push = shell_push
M._shell_redraw = shell_redraw
M._shell_select = shell_select
M._shell_is_open = shell_is_open
M._bmap = bmap
M._km = km
M._pad = pad
M._TAB_BAR_LINES = TAB_BAR_LINES
M._LIST_HEADER_LINES = LIST_HEADER_LINES
